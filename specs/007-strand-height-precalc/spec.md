...
- **FR-001**: System MUST provide a selectable placement mode (distinct from the default) that, before placing strands, precalculates **for each of the two strands an opening introduces** the maximum vertical row that strand will occupy between its opening and its matching closing. The two strands of a pair MAY have different maxima, because an opening occurring between them raises the upper strand without raising the lower. The maximum MUST be taken over the strand's **flat run only** — the rows it occupies from the end of its opening transfer to the start of its closing transfer — and MUST NOT include the boundary movement by which the strand meets its cap or cup (FR-015, FR-016). Excluding boundary movement is what keeps this a single forward pass: were it included, a strand's maximum would depend on a cap/cup row itself computed from that maximum.
- **FR-002**: System MUST place each opening feature unconditionally at the midpoint of its two strands' precalculated maxima, floored to a whole row — `floor((lower_max + upper_max) / 2)` — rather than at the lowest free row, and without weighing crossing-alignment cost against displacement savings. Flooring is correct rather than approximate: the opening glyph renders halfway up its tile, so the tile at `floor(m)` already places the cap at height `m` for any midpoint ending in `.5`.
...
- **Maximum strand row (precalculated height)**: the highest vertical row a **single strand** occupies over its **flat run** — from the end of its opening transfer to the start of its closing transfer — which is the row that strand settles at in the new mode. Boundary movement to meet a cap or cup is excluded by definition (FR-001), so the value never depends on a placement derived from it. Calculated per strand, so the two strands of one pair may have different values.
