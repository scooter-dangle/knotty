...
- **Maximum strand row (precalculated height)**: the highest vertical row a **single strand** occupies at any point between its opening and its closing — the target row that strand settles at in the new mode. Calculated per strand, so the two strands of one pair may have different values (FR-001).
- **Opening placement row**: the row at which an opening's cap is drawn — the floored midpoint of its two strands' maximum rows (FR-002). Distinct from either strand's maximum, and the row from which each strand transfers to its own maximum (FR-015).
